...
struct colorscheme {
	...
	char *jobs_color;   char *preposition_color;
};
enum Colorschemes {
	KANAGAWA_WAVE,
	KANAGAWA_LOTUS
...
const Colorscheme colors[] = {
	[KANAGAWA_WAVE] = {
		.text_color = COLOR_8(7),           .username_color = BRIGHT_COLOR_8(3),        .hostname_color      = RGB(210, 126, 153),
		.cwd_color  = COLOR_8(2),           .time_color     = BRIGHT_COLOR_8(5),        .hist_count_color    = RGB(89, 123, 117),
		.git_color  = RGB(255, 160,102),    .jobs_color     = RGB(147, 138, 169),       .preposition_color   = COLOR_256(240)
	},
	[KANAGAWA_LOTUS] = {
		.text_color = COLOR_8(7),           .username_color = BRIGHT_COLOR_8(3),        .hostname_color     = RGB(210, 126, 153),
		.cwd_color  = BRIGHT_COLOR_8(2),    .time_color	    = BRIGHT_COLOR_8(5),        .hist_count_color   = RGB(89, 123, 117),
		.git_color  = RGB(255, 160, 102),   .jobs_color     = RGB(147, 138, 169),       .preposition_color  = COLOR_256(21)
	}
};
...
{
	if(strstr(distro, "Ubuntu")) distro = UBUNTU_ICON_COLOR" ";
	if(strstr(distro, "Debian")) distro = DEBIAN_ICON_COLOR" ";
...
void print_jobs(char *background_str, char *stopped_str)
{
	...
	if (background > 0 || stopped > 0)  start += snprintf(start, end - start, colorscheme->jobs_color);
	if (background > 0 && stopped > 0)  start += snprintf(start, end - start, " %d 󰓦 / %d  ", background, stopped);
	else if (background > 0)            start += snprintf(start, end - start, " %d 󰓦", background);
	else if (stopped > 0)               start += snprintf(start, end - start, " %d  ", stopped);
}
...
int main(int argc, char **argv)
{
	...
	for(int i=2; i<argc; i++) {
		...
		case 'H':
			...
			break;
		case 'g':
			if(argv[i][0]) print_git_branch(argv[i]);
			break;
		case 'j':
